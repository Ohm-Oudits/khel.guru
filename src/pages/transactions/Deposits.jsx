  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setDeposits(transactions.filter((txn) => txn.type === "deposit"));
      } catch {
        if (!cancelled) setError("Could not load deposits.");
      } finally {
  ...
  return (
    <main className="m p-6 max-w-[1200px] flex justify-between mx-auto text-white">
      ...
      <section className="max-w-[900px] w-full mx-auto bg-gray-900 p-4 py-6 rounded-md">
        <h2 className="px-2 text-lg font-bold">Deposits</h2>
        <div className=" rounded-lg mt-4">
          {loading ? (
            <p className="p-3 text-gray-400">Loading deposits...</p>
          ) : error ? (
            <p className="p-3 text-gray-400">{error}</p>
          ) : deposits.length === 0 ? (
            <p className="p-3 text-gray-400">No deposits yet.</p>
          ) : (
            deposits.map((txn, index) => (
              <div
                key={txn._id || index}
              ...
              >
                ...
                <div className="flex items-center space-x-2 text-gray-300">
                  <span className="uppercase text-xs text-gray-500">
                    {txn.meta?.method || txn.meta?.chain || "cash"}
                  </span>
